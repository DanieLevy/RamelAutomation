// Modern & Improved Email Templates for Tor-RamEl - Node.js Compatible Version
// Design focuses on clarity, trust, and a premium user experience.
// Fully responsive with dark mode support.

// =================================================================================
// HELPER FUNCTIONS (No changes needed, functionality preserved)
// =================================================================================
...
// =================================================================================
// DATA CONVERSION LOGIC (For testing, functionality preserved)
// =================================================================================

/**
 * Converts test data into the format expected by the email template function.
 * This allows for easy testing with different data structures.
 */
const convertTestDataToEmailFormat = (data) => {
  const commonData = {
    notificationCount: data.totalNotificationCount || 0,
    unsubscribeUrl: 'https://tor-ramel.netlify.app/unsubscribe?email=test@example.com', // Example URL
    userEmail: 'daniellofficial@gmail.com' // Example email
  };

  if (data.appointment) {
    // Single appointment structure
    return {
      ...commonData,
      matchingResults: [{
        date: data.appointment.date,
        available: true,
        times: [data.appointment.time]
      }],
      criteriaType: 'single'
    };
  } else {
    // Multiple appointments structure
    return {
      ...commonData,
      matchingResults: data.appointments.map(apt => ({
        date: apt.date,
        available: true,
        times: [apt.time]
      })),
 ...
 * @param {Array<object>} params.matchingResults - Array of available appointments.
 * @param {number} params.notificationCount - The number of prior notifications sent.
 * @param {string} params.unsubscribeUrl - The URL for the user to unsubscribe.
 * @param {string} params.userEmail - The recipient's email address.
 * @returns {{subject: string, html: string, text: string}} The complete email object.
 */
const generateModernEmailTemplate = ({
  matchingResults,
  notificationCount,
  unsubscribeUrl,
  userEmail,
  ...
  const mainDate = matchingResults[0].date;
  const hebrewMainDate = formatHebrewDate(mainDate);
  const dayName = getDayNameHebrew(mainDate);
...
// =================================================================================
// MODULE EXPORTS (Preserved for compatibility)
// =================================================================================

module.exports = {
  generateModernEmailTemplate,
  generateEmailContent: (testData) => {
    const emailData = convertTestDataToEmailFormat(testData);
    return generateModernEmailTemplate(emailData);
  }
};
